%==============================================
% WorldTheory.pl for actor operator
%==============================================
/*
For a QActor as a singleton statically degined in the model
*/
myname(qatuoperator).	%%old version (deprecated)
actorobj(qatuoperator).	%% see registerActorInProlog18 in QActor
...
%-------------------------------------------------
%  TuProlo FEATURES of the QActor operator
%-------------------------------------------------
dialog( FileName ) :-  
...
%% :- stdout <- println(  "hello from world theory of operator" ). 
...
%-------------------------------------------------
%  User static rules about operator
%------------------------------------------------- 
/*
------------------------------------------------------------------------
...
actorOp( Op )   :- % actorPrintln( actorOp( Op  ) ),
				   java_catch(
						qatuoperator <- Op returns R, 	%% R unbound if void
...
%% setActionResult : qatuoperator register (in Java) the result under name 'actoropresult'
...
cvtToString( V , "void" ):-
	unbound(V),!,
	qatuoperator <- setActionResult( void ).
cvtToString( V , S ):-
	number(V),!,
	setResult(V),
	text_term( S, V ).
cvtToString( V , S ):-
	...
	qatuoperator <- setActionResult( V ),
	!.
actorOpResult( V )   :- result(V),!.	
actorOpResult( Res ) :- actoropresult <- toString returns Res, !.
%% actorOpResult( "" ).

androidConsult(T) :- qatuoperator <- androidConsult(T), !.
