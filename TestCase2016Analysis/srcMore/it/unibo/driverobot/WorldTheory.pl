%==============================================
% WorldTheory.pl for actor driverobot
%==============================================
/*
For a QActor as a singleton statically degined in the model
*/
myname(qatudriverobot).	%%old version (deprecated)
actorobj(qatudriverobot).	%% see registerActorInProlog18 in QActor
...
%-------------------------------------------------
%  TuProlo FEATURES of the QActor driverobot
%-------------------------------------------------
dialog( FileName ) :-  
...
%% :- stdout <- println(  "hello from world theory of driverobot" ). 
...
loop(I,N,Op) :-  
		...
		%%output( values( I,V,N ) ),
 		V =< N ,!,
   		%% qatudriverobot <- println( loop( I,V ) ),
...
%-------------------------------------------------
%  User static rules about driverobot
%------------------------------------------------- 
/*
------------------------------------------------------------------------
...
actorOp( Op )   :- % actorPrintln( actorOp( Op  ) ),
				   java_catch(
						qatudriverobot <- Op returns R, 	%% R unbound if void
...
%% setActionResult : qatudriverobot register (in Java) the result under name 'actoropresult'
...
cvtToString( V , "void" ):-
	unbound(V),!,
	qatudriverobot <- setActionResult( void ).
cvtToString( V , S ):-
	number(V),!,
	setResult(V),
	text_term( S, V ).
cvtToString( V , S ):-
	...
	qatudriverobot <- setActionResult( V ),
	!.
actorOpResult( V )   :- result(V),!.	
actorOpResult( Res ) :- actoropresult <- toString returns Res, !.
%% actorOpResult( "" ).

androidConsult(T) :- qatudriverobot <- androidConsult(T), !.
