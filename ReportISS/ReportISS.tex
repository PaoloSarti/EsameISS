...
The \textbf{led} blinking logic is implemented directly as QActor behaviour. The Prolog theory turnTheLed(X) allows the QActor to manage the led and actually turn it on and off.
\lstinputlisting[language=prolog, frame=single]{../TestCase2016Project/src/it/unibo/led/ledTheory.pl}
The \textbf{operator console} includes a GUI that translates external input events in messages to drive the robot.
...
The \textbf{robot} uses a Prolog theory to execute the detection phase. It takes a picture of the bag (in our implementation, it simulates this behaviour) and sends it to the ASC. Some Java library has to be called to convert the image as a string and vice versa.\\
